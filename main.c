...
/*   Updated: 2025/04/09 17:55:47 by aldferna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_data(t_data *info, int argc, char **argv)
{
	...
	if (argc != 5 && argc != 6)
		return (printf("num of philosp - time die - time eat - time sleep
				- [num meals]\n"), 0);
	i = 1;
	while (argv[i])
	{
		...
		if (!ft_isdigit(argv[i]) || !argv[i]) // solo numeros
			return (printf("nnnum of philosp - time die - time eat - time sleep
					- num meals\n"), 0);
		i++;
	}
